...
class Main {
    public static void main(String[] args) throws IOException {
        ...
        String input = String.valueOf(Integer.parseInt(br.readLine()) * Integer.parseInt(br.readLine()) * Integer.parseInt(br.readLine()));
        int[] arr = new int[10];

        for (int i = 0; i < input.length(); i++) {
          char c = input.charAt(i);
          for (int j = 0; j < 10; j++) if(Integer.parseInt(String.valueOf(c)) == j) arr[j]++;
        }
